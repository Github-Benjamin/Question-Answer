...
# 检查注册用户名规则
def CheckUserData(username,password,password1,email):
    if (username and password and password1 and email):
        if CheckChina(username):
            return '<h3>用户名不能包含中文字符，请重新输入</h3>'
        if password != password1:
            return '<h3>两次密码输入不一致，请重新输入</h3>'
        if CheckChina(password):
            return '<h3>密码不能包含中文</h3>'
        if len(username) > 10:
            return '<h3>用户名长度最大为10字符，请重新输入</h3>'
        if len(password) > 16:
            return '<h3>密码长度最长为16个字符，请重新输入</h3>'
        if not validateEmail(email):
            return '<h3>邮件格式不合法，请重新输入</h3>'
        return 1
    else:
        return '<h3>数据不全，请重新输入</h3>'